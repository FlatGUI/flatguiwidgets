...
(def max-diagn-out-strlen 400)

(defn- kv->str [[k v]]
  (condp = k
    :children (str k " count = " (count v))
    :evolvers (str k " count = " (count v))
    (let [v-str (str v)] (str k " " (if (> (.length v-str) max-diagn-out-strlen) (str (subs v-str 0 max-diagn-out-strlen) "...") v-str)))))

(defn print-diagnostics [component]
  (do
    (println "------------------" (:id component) "------------------")
    (println (map (fn [kv] (str (kv->str kv) \newline)) (sort-by key component)))))

(fg/defevolverfn :diagnostics-requested
  (let [new-value (if (= 3 (mouse/get-click-count component))
                    true
                    false)
        _ (if (and (not= old-diagnostics-requested new-value) new-value)
            (print-diagnostics component))]
    new-value))
...
(fg/defwidget "componentbase"
  (array-map
    ...
    :diagnostics-requested false
    :evolvers {:diagnostics-requested diagnostics-requested-evolver
               :interop interop-evolver
               :theme theme-evolver
               :skin skin-evolver
               :font font-evolver
